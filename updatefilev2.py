...
def init_db():
    ...
    if "pending_message_id" not in existing_cols:
        cur.execute("ALTER TABLE deposits ADD COLUMN pending_message_id INTEGER DEFAULT 0")
    if "pending_chat_id" not in existing_cols:
        cur.execute("ALTER TABLE deposits ADD COLUMN pending_chat_id INTEGER DEFAULT 0")
...
def _telegram_edit_message(chat_id, message_id, text):
    """Edit an existing Telegram message from the webhook thread."""
    if not BOT_TOKEN or not chat_id or not message_id:
        return False
    try:
        payload = json.dumps({"chat_id": int(chat_id), "message_id": int(message_id), "text": text, "parse_mode": "HTML"}).encode("utf-8")
        req = Request(f"https://api.telegram.org/bot{BOT_TOKEN}/editMessageText", data=payload, headers={"Content-Type":"application/json"}, method="POST")
        with urlopen(req, timeout=15) as response:
            return 200 <= response.status < 300
    except Exception:
...
def auto_reject_payment(provider, amount, trx_id, raw_sms):
    """Reject a pending deposit when the received SMS proves the submitted TRX is not the received TRX."""
    ...
    try:
        con.execute("BEGIN IMMEDIATE")
        by_trx = con.execute("SELECT d.*, u.username, u.first_name FROM deposits d LEFT JOIN users u ON u.user_id=d.user_id WHERE d.status='pending' AND UPPER(TRIM(d.trx_id))=? ORDER BY d.id ASC LIMIT 1", (trx_norm,)).fetchone()
        if by_trx and (normalize_payment_method(by_trx["payment_method"]) != provider or abs(float(by_trx["amount"]) - amount) >= 0.005):
            row = by_trx
        else:
            row = con.execute("SELECT d.*, u.username, u.first_name FROM deposits d LEFT JOIN users u ON u.user_id=d.user_id WHERE d.status='pending' AND LOWER(TRIM(d.payment_method)) LIKE ? AND ABS(d.amount - ?) < 0.005 ORDER BY d.id ASC LIMIT 1", (f"%{provider}%", amount)).fetchone()
        if not row:
            con.rollback()
            return {"ok": False, "reason": "no_candidate_to_reject"}
        con.execute("UPDATE deposits SET status='rejected', auto_processed=1, auto_processed_at=? WHERE id=? AND status='pending'", (now(), row["id"]))
        con.commit()
    except Exception:
        con.rollback()
        logger.exception("Auto payment rejection failed")
        return {"ok": False, "reason": "database_error"}
    finally:
        con.close()
    cancel_text = (f"❌ <b>Deposit Cancelled</b>\n\n" f"💳 Method: <b>{escape(row['payment_method'] or provider)}</b>\n" f"💰 Amount: <b>{money(row['amount'])} ৳</b>\n" f"🧾 Submitted TRX: <code>{escape(row['trx_id'])}</code>\n\n" f"Payment SMS-এর Transaction ID আপনার submitted Transaction ID-এর সাথে match করেনি।\n" f"Deposit request automatically cancelled হয়েছে।")
    if row["pending_chat_id"] and row["pending_message_id"]:
        _telegram_edit_message(row["pending_chat_id"], row["pending_message_id"], cancel_text)
    else:
        _telegram_send_message(row["user_id"], cancel_text)
    return {"ok": True, "status": "rejected", "deposit_id": row["id"], "user_id": row["user_id"], "amount": row["amount"], "provider": provider, "trx_id": row["trx_id"]}
...
def auto_approve_payment(provider, amount, trx_id, raw_sms):
    ...
    try:
        ...
        if not rows:
            con.rollback()
            return auto_reject_payment(provider, amount, trx_id, raw_sms)
    ...
    # Replace the user's processing message with the final success state.
    success_text = (
        ...
        f"💵 New Balance: <b>{money(new_balance)} ৳</b>\n\n"
        f"Payment successfully verified and balance updated."
    )
    if row["pending_chat_id"] and row["pending_message_id"]:
        _telegram_edit_message(row["pending_chat_id"], row["pending_message_id"], success_text)
    else:
        _telegram_send_message(row["user_id"], success_text)
...
async def deposit_trx(update, context, trx):
    ...
    processing_message = await update.message.reply_text(
        premiumize_html(
            ...
            f"💰 Amount: <b>{money(amount)} ৳</b>\n"
            f"🧾 TRX: <code>{escape(trx)}</code>\n\n"
            f"Payment SMS-এর সাথে আপনার Transaction ID match করা হচ্ছে।\n"
            f"অনুগ্রহ করে অপেক্ষা করুন।"
        ),
        parse_mode="HTML"
    )
    con = db()
    con.execute("UPDATE deposits SET pending_message_id=?, pending_chat_id=? WHERE id=?", (processing_message.message_id, user.id, deposit_id))
    con.commit()
    con.close()
