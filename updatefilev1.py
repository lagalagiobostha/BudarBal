...
def init_db():
    ...
    if "processing_chat_id" not in existing_cols:
        cur.execute("ALTER TABLE deposits ADD COLUMN processing_chat_id INTEGER DEFAULT 0")
...
def _telegram_edit_message(chat_id, message_id, text):
    """Edit the existing payment-processing message instead of sending a second one."""
    ...
    try:
        ...
        logger.exception("Telegram processing-message edit failed")
        return False


def auto_approve_payment(provider, amount, trx_id, raw_sms):
    """
    Match an incoming SMS against a pending deposit.

    Approval requires an exact provider + amount + transaction-ID match.
    If the provider and amount match a single pending deposit but the
    submitted transaction ID does not, that pending request is immediately
    cancelled as a transaction-ID mismatch.
    """
    ...
    try:
        ...
        if exact:
            ...
            edited = _telegram_edit_message(row["processing_chat_id"], row["processing_message_id"], success_text)
            if not edited:
            ...
            return {
                "ok": True, "status": "approved", "deposit_id": row["id"],
                "user_id": row["user_id"], "amount": row["amount"],
                "provider": provider, "trx_id": row["trx_id"]
            }

        # No exact transaction-ID match. Only cancel automatically when there
        # is exactly one pending request for this provider + amount. This avoids
        # cancelling another user's request when multiple identical deposits exist.
        candidates = con.execute("""
            ...
            WHERE d.status='pending'
              ...
              AND ABS(d.amount - ?) < 0.005
            ORDER BY d.id DESC
        """, (f"%{provider}%", amount)).fetchall()

        if len(candidates) != 1:
            con.rollback()
            return {
                "ok": False,
                "reason": "trx_mismatch_ambiguous" if len(candidates) > 1 else "no_pending_same_amount"
            }

        row = candidates[0]
        con.execute("""
            UPDATE deposits
            SET status='rejected', auto_processed=1, auto_processed_at=?
            WHERE id=? AND status='pending'
        """, (now(), row["id"]))
        con.commit()

        cancelled_text = premiumize_html(
            f"❌ <b>Payment Cancelled</b>\n\n"
            f"💳 Method: <b>{escape(row['payment_method'] or provider)}</b>\n"
            f"💰 Amount: <b>{money(row['amount'])} ৳</b>\n"
            f"🧾 Submitted TRX: <code>{escape(row['trx_id'])}</code>\n\n"
            f"আপনার দেওয়া Transaction ID-এর সাথে payment SMS-এর Transaction ID মেলেনি।\n"
            f"Deposit request বাতিল করা হয়েছে। সঠিক Transaction ID দিয়ে নতুন করে চেষ্টা করুন।"
        )
        edited = _telegram_edit_message(row["processing_chat_id"], row["processing_message_id"], cancelled_text)
        if not edited:
            _telegram_send_message(row["user_id"], cancelled_text)

        return {
            "ok": True, "status": "rejected", "deposit_id": row["id"],
            "user_id": row["user_id"], "amount": row["amount"],
            "provider": provider, "submitted_trx": row["trx_id"],
            "received_trx": trx_norm
        }
...
async def deposit_trx(update, context, trx):
    ...
    cur = con.execute("""
        ...
        VALUES (?, ?, ?, ?, 'pending', ?)
    """, (user.id, amount, method, trx, now()))
    deposit_id = cur.lastrowid
    con.commit()
    con.close()

    # Keep the admin-side pending deposit notification unchanged.
    ...
    if AUTO_PAYMENT_ENABLED:
        processing_text = premiumize_html(
            f"⏳ <b>Payment Processing</b>\n\n"
            f"💳 Method: <b>{escape(method)}</b>\n"
            f"💰 Amount: <b>{money(amount)} ৳</b>\n"
            f"🧾 TRX: <code>{escape(trx)}</code>\n\n"
            f"আপনার payment যাচাই করা হচ্ছে। অনুগ্রহ করে অপেক্ষা করুন।\n"
            f"Transaction ID match হলে payment automatically approve হবে।"
        )
        ...
        con = db()
        con.execute(
            "UPDATE deposits SET processing_chat_id=?, processing_message_id=? WHERE id=?",
            (user.id, sent.message_id, deposit_id)
        )
        con.commit()
        con.close()
    else:
        await update.message.reply_text(
            premiumize_html(
                ...
                "Payment verification-এর জন্য Admin review প্রয়োজন।"
            ),
            parse_mode="HTML",
            reply_markup=main_keyboard(user.id)
        )
