...
def init_db():
    ...
    if "processing_chat_id" not in existing_cols:
        cur.execute("ALTER TABLE deposits ADD COLUMN processing_chat_id INTEGER DEFAULT NULL")
    if "processing_message_id" not in existing_cols:
        cur.execute("ALTER TABLE deposits ADD COLUMN processing_message_id INTEGER DEFAULT NULL")
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("deposit_approve:") or data.startswith("deposit_reject:"):
        ...
        if action == "deposit_approve":
            ...
            con.close()
            try:
                if row["processing_chat_id"] and row["processing_message_id"]:
                    await context.bot.delete_message(row["processing_chat_id"], row["processing_message_id"])
            except Exception:
                pass
            try:
                new_balance = get_balance(row["user_id"])
                await context.bot.send_message(
                    row["user_id"], deposit_approved_text(row["amount"], row["payment_method"], new_balance), parse_mode="HTML"
                )
            except Exception:
                pass
            ...
            await query.edit_message_text(
                premiumize_html(f"✅ Deposit #{did} approved.\nAmount: {money(row['amount'])} ৳"), parse_mode="HTML"
            )
        else:
            con.execute(
            ...
            try:
                ...
                pass
            try:
                await context.bot.send_message(row["user_id"], deposit_rejected_text(row["amount"]), parse_mode="HTML")
            except Exception:
                pass
            await query.edit_message_text(
...
def deposit_approved_text(amount, method, new_balance):
    return premiumize_html(
        f"🎉 <b>Deposit Approved!</b>\n\n"
        f"💰 <b>Amount:</b> {money(amount)} ৳\n"
        f"💳 <b>Method:</b> {escape(method or '—')}\n"
        f"💵 <b>New Balance:</b> {money(new_balance)} ৳\n\n"
        f"❤️ আমাদের বেছে নেওয়ার জন্য ধন্যবাদ।\n\n"
        f"🛍️ এখন আপনি <b>Buy Product</b>-এ ক্লিক করে আপনার পছন্দমতো Product ক্রয় করতে পারবেন।"
    )

def deposit_processing_text():
    return premiumize_html(
        "⏳ <b>Processing...</b>\n\n"
        "আপনার পেমেন্টটি যাচাই করা হচ্ছে।\n"
        "অতি শীঘ্রই <b>Automatic Payment Processor</b> অথবা <b>Admin</b> দ্বারা কনফার্ম করা হবে।"
    )

def deposit_rejected_text(amount):
    return premiumize_html(
        f"❌ <b>Deposit Rejected!</b>\n\n"
        f"💰 <b>Amount:</b> {money(amount)} ৳\n\n"
        "দুঃখিত, আপনার Deposit Request টি Reject করা হয়েছে।"
    )
...
def _telegram_delete_message(chat_id, message_id):
    if not BOT_TOKEN or not chat_id or not message_id:
        return False
    try:
        payload = json.dumps({"chat_id": int(chat_id), "message_id": int(message_id)}).encode("utf-8")
        req = Request(
...
def auto_approve_payment(provider, amount, trx_id, raw_sms):
    ...
    # Remove the user's Processing message, then send the same success message
    # used by manual approval.
    _telegram_delete_message(row["processing_chat_id"], row["processing_message_id"])
    _telegram_send_message(
        row["user_id"],
        deposit_approved_text(row["amount"], row["payment_method"] or provider, new_balance)
    )
...
async def deposit_trx(update, context, trx):
    ...
    # Show one Processing message and remember its Telegram message ID so it can
    # be removed immediately when manual or automatic approval/rejection happens.
    context.user_data.clear()
    try:
        processing = await update.message.reply_text(
            deposit_processing_text(), parse_mode="HTML", reply_markup=main_keyboard(user.id)
        )
        con = db()
        cur = con.execute(
            "UPDATE deposits SET processing_chat_id=?, processing_message_id=? WHERE id=? AND status='pending'",
            (user.id, processing.message_id, deposit_id)
        )
        con.commit()
        status_row = con.execute("SELECT status FROM deposits WHERE id=?", (deposit_id,)).fetchone()
        con.close()
        # If auto approval won the race before the processing message was saved,
        # remove the now-stale message immediately. Auto approval already sent the
        # final success message.
        if not cur.rowcount or (status_row and status_row["status"] != "pending"):
            try:
                await processing.delete()
            except Exception:
                pass
    except Exception:
        logger.exception("Failed to create Processing message")
        await update.message.reply_text(deposit_processing_text(), parse_mode="HTML", reply_markup=main_keyboard(user.id))
...
async def approve_deposit(update, context):
    ...
    try:
        await context.bot.send_message(
            row["user_id"],
            deposit_approved_text(row["amount"], row["payment_method"], get_balance(row["user_id"])),
            parse_mode="HTML"
        )
    except Exception:
        pass
...
async def reject_deposit(update, context):
    ...
    con = db()
    row = con.execute("SELECT * FROM deposits WHERE id=? AND status='pending'", (did,)).fetchone()
    if not row:
        con.close()
        await update.message.reply_text(premiumize_html("❌ Deposit not found or already processed."), parse_mode="HTML")
        return
    con.execute("UPDATE deposits SET status='rejected' WHERE id=? AND status='pending'", (did,))
    con.commit()
    con.close()
    try:
        if row["processing_chat_id"] and row["processing_message_id"]:
