...
def init_db():
    ...
    if "pending_message_id" not in existing_cols:
        cur.execute("ALTER TABLE deposits ADD COLUMN pending_message_id INTEGER DEFAULT NULL")
    if "pending_chat_id" not in existing_cols:
        cur.execute("ALTER TABLE deposits ADD COLUMN pending_chat_id INTEGER DEFAULT NULL")
        ...
        if action == "deposit_approve":
            ...
            try:
                if row["pending_message_id"]:
                    await context.bot.delete_message(chat_id=row["user_id"], message_id=row["pending_message_id"])
            except Exception:
                pass
            try:
                await context.bot.send_message(
...
def _telegram_delete_message(chat_id, message_id):
    """Delete a user's pending deposit message from the webhook thread."""
    ...
    try:
        ...
        logger.exception("Telegram pending deposit message deletion failed")
        return False
...
def auto_approve_payment(provider, amount, trx_id, raw_sms):
    ...
    try:
        ...
        pending_message_id = row["pending_message_id"] if "pending_message_id" in row.keys() else None
        pending_chat_id = row["pending_chat_id"] if "pending_chat_id" in row.keys() else row["user_id"]
    ...
    # Remove the persistent pending message before sending the final result.
    _telegram_delete_message(pending_chat_id, pending_message_id)
...
async def deposit_trx(update, context, trx):
    ...
    # Keep a single pending message visible until the deposit is resolved.
    con = db()
    ...
    if current_status == "approved":
        response_text = (
            ...
            "💳 আপনার balance স্বয়ংক্রিয়ভাবে update হয়েছে।"
        )
        await update.message.reply_text(premiumize_html(response_text), parse_mode="HTML", reply_markup=main_keyboard(user.id))
        return

    response_text = (
        "⏳ <b>Payment Pending</b>\n\n"
        "আপনার payment verification-এর জন্য অপেক্ষমাণ রয়েছে।\n"
        "🔐 Verification সম্পন্ন হলে balance স্বয়ংক্রিয়ভাবে আপডেট হবে।"
    )
    pending_message = await update.message.reply_text(
        premiumize_html(response_text), parse_mode="HTML", reply_markup=main_keyboard(user.id)
    )

    con = db()
    con.execute(
        "UPDATE deposits SET pending_message_id=?, pending_chat_id=? WHERE id=? AND status='pending'",
        (pending_message.message_id, user.id, deposit_id)
    )
    con.commit()
    final_row = con.execute("SELECT status FROM deposits WHERE id=?", (deposit_id,)).fetchone()
    con.close()
    if final_row and final_row["status"] != "pending":
        try:
            await context.bot.delete_message(chat_id=user.id, message_id=pending_message.message_id)
        except Exception:
            pass
...
async def reject_deposit(update, context):
    ...
    if not row:
        con.close()
        await update.message.reply_text(premiumize_html("❌ Deposit not found."), parse_mode="HTML")
        return
    con.execute("UPDATE deposits SET status='rejected' WHERE id=?", (did,))
    con.commit(); con.close()
    ...
    try:
        await context.bot.send_message(
            row["user_id"],
            premiumize_html(f"❌ <b>Deposit Rejected</b>\n\nAmount: <b>{money(row['amount'])} ৳</b>"),
            parse_mode="HTML"
        )
    except Exception:
        pass
    await update.message.reply_text(premiumize_html("❌ Deposit rejected."), parse_mode="HTML")
