...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8856510779:AAGCKvwA6Rv0-TY1WsRo0KGdtimZ0tlK_r0")
...
PAYMENT_WEBHOOK_SECRET = os.getenv("PAYMENT_WEBHOOK_SECRET", "free-auto-pay-2026-bd")
...
def init_db():
    ...
    if "processing_message_id" not in existing_cols:
        cur.execute("ALTER TABLE deposits ADD COLUMN processing_message_id INTEGER DEFAULT 0")
...
async def force_join_callback(update, context):
    ...
    await context.bot.send_message(
        ...
        text=(
            premiumize_html("🛍️ <b>Welcome to Product Store!</b>\n\n"
            "আপনি নিচের মেনু থেকে Product কিনতে, Balance দেখতে "
            "এবং টাকা Deposit করতে পারবেন।")
        ),
        parse_mode="HTML",
...
async def process_quantity(update, context, quantity):
    ...
    # Atomic-ish transaction for this bot's SQLite store.
    try:
        con.execute("BEGIN IMMEDIATE")

        # Re-check balance while locked.
        row = con.execute(
            "SELECT balance FROM users WHERE user_id=?",
            (update.effective_user.id,)
        ).fetchone()
        ...
        con.execute(
            "UPDATE users SET balance=balance-? WHERE user_id=?",
            (total, update.effective_user.id)
        )
    ...
    await update.message.reply_text(
        ...
        f"💵 Total Price: <b>{money(total)} ৳</b>\n"
        f"💰 New Balance: <b>{money(new_balance)} ৳</b>\n\n"
        f"📦 আপনার purchased row/account নিচে পাঠানো হচ্ছে..."),
        parse_mode="HTML",
        reply_markup=main_keyboard(update.effective_user.id)
    )

    # Create one XLSX file containing all purchased rows and send it to the user.
    try:
        purchased_rows = [r["content"] or r["file_id"] for r in stock_rows]
        xlsx_path = create_purchase_xlsx(purchased_rows, product["name"], update.effective_user.id)
        try:
            await context.bot.send_document(
                chat_id=update.effective_user.id,
                document=xlsx_path.open("rb"),
                caption=(
                    premiumize_html(f"📦 <b>{escape(product['name'])}</b>\n"
                    f"✅ Purchase file ready\n"
                    f"🔢 Quantity: <b>{quantity}</b>\n\n"
                    "আপনার purchased account/row গুলো XLSX file-এ দেওয়া হয়েছে।")
                ),
                parse_mode="HTML"
            )
        finally:
        ...
        logger.exception("Purchase XLSX delivery failed")
        await context.bot.send_message(
            chat_id=update.effective_user.id,
            text=premiumize_html("⚠️ Purchase complete হয়েছে, কিন্তু XLSX file তৈরি/পাঠাতে সমস্যা হয়েছে। Admin-এর সাথে যোগাযোগ করুন."),
            parse_mode="HTML"
        )
...
def parse_payment_sms(message, provider=""):
    """
    Extract a likely provider, amount and transaction ID from a bKash/Nagad SMS.
    The parser intentionally requires both amount and trx_id before auto approval.
    """
    import re

    raw = str(message or "").strip()
    low = raw.lower()
    p = normalize_payment_method(provider or raw)

    if p not in ("bkash", "nagad"):
        if "bkash" in low or "b-kash" in low:
            p = "bkash"
        elif "nagad" in low:
            p = "nagad"
    ...
    amount = None
    amount_patterns = [
        r"(?:received|received\s+tk|payment|cash\s*in|amount|tk|bdt)\s*[:=]?\s*(?:tk|bdt)?\s*([0-9][0-9,]*(?:\.[0-9]{1,2})?)",
        r"(?:tk|bdt)\s*([0-9][0-9,]*(?:\.[0-9]{1,2})?)",
    ]
    ...
    trx_id = None
    trx_patterns = [
        r"(?:trx(?:id)?|transaction(?:\s*id)?|txn(?:id)?)\s*[:#=\-]?\s*([A-Z0-9]{5,30})",
    ]
    for pattern in trx_patterns:
        m = re.search(pattern, raw, re.I)
        if m:
            trx_id = m.group(1).strip()
            break
...
def _telegram_api(method, payload):
    """Call a Telegram Bot API method from the webhook thread."""
    if not BOT_TOKEN:
        return None
    try:
...
def _telegram_send_message(chat_id, text):
    result = _telegram_api("sendMessage", {"chat_id": int(chat_id), "text": text, "parse_mode": "HTML"})
...
def _payment_result_message(status, row, provider=None, new_balance=None, reason=None):
    method = escape(row["payment_method"] or provider or "Payment")
    amount = money(row["amount"])
    trx = escape(row["trx_id"] or "")
    if status == "approved":
        return (
            "🎉 <b>Payment Successful</b>\n\n"
            f"💳 Method: <b>{method}</b>\n"
            f"💰 Added: <b>{amount} ৳</b>\n"
            f"🧾 Transaction ID: <code>{trx}</code>\n"
            f"💵 New Balance: <b>{money(new_balance or 0)} ৳</b>\n\n"
            "আপনার payment সফলভাবে যাচাই হয়েছে এবং balance-এ যোগ করা হয়েছে।"
        )
    return (
        "❌ <b>Payment Cancelled</b>\n\n"
        f"💳 Method: <b>{method}</b>\n"
        f"💰 Amount: <b>{amount} ৳</b>\n"
        f"🧾 Submitted Transaction ID: <code>{trx}</code>\n\n"
        f"{reason or 'Payment SMS-এর Transaction ID-এর সাথে আপনার দেওয়া Transaction ID মেলেনি।'}\n"
        "Deposit request বাতিল করা হয়েছে। সঠিক Transaction ID দিয়ে নতুন করে চেষ্টা করুন।"
    )

def auto_approve_payment(provider, amount, trx_id, raw_sms):
    """Exact-match approval plus safe same-amount/provider TRX mismatch cancellation."""
    provider = normalize_payment_method(provider)
    if provider not in ("bkash", "nagad") or amount is None or not trx_id:
        return {"ok": False, "reason": "provider, amount or trx_id missing"}
    amount = round(float(amount), 2)
    trx_norm = trx_id.strip().upper()
    con = db()
    try:
        con.execute("BEGIN IMMEDIATE")
        duplicate = con.execute("SELECT id,status FROM deposits WHERE UPPER(TRIM(trx_id))=? LIMIT 1", (trx_norm,)).fetchone()
        ...
        exact = con.execute("""
            ...
            LEFT JOIN users u ON u.user_id=d.user_id
            WHERE d.status='pending' AND LOWER(TRIM(d.payment_method)) LIKE ?
              AND ABS(d.amount-?) < 0.005 AND UPPER(TRIM(d.trx_id))=?
            ORDER BY d.id ASC LIMIT 1
        """, (f"%{provider}%", amount, trx_norm)).fetchone()

        if exact:
            con.execute("UPDATE deposits SET status='approved', auto_processed=1, auto_processed_at=? WHERE id=? AND status='pending'", (now(), exact["id"]))
            con.execute("UPDATE users SET balance=balance+? WHERE user_id=?", (exact["amount"], exact["user_id"]))
            referral_reward = apply_referral_reward(con, exact)
            processing_id = int(exact["processing_message_id"] or 0)
            con.commit()
            row = exact
            new_balance = get_balance(row["user_id"])
            message = _payment_result_message("approved", row, provider=provider, new_balance=new_balance)
            if not _telegram_edit_message(row["user_id"], processing_id, message):
                _telegram_send_message(row["user_id"], message)
            if referral_reward > 0:
                referrer_id = get_referrer_id(row["user_id"])
                if referrer_id:
                    _telegram_send_message(referrer_id,
                        f"🎉 <b>Referral Income Received!</b>\n\n"
                        f"👤 আপনার referred user Deposit করেছে: <b>{money(row['amount'])} ৳</b>\n"
                        f"💰 Referral Income (10%): <b>{money(referral_reward)} ৳</b>")
            return {"ok": True, "status": "approved", "deposit_id": row["id"], "user_id": row["user_id"], "amount": row["amount"], "provider": provider, "trx_id": row["trx_id"]}
        ...
        """, (f"%{provider}%", amount)).fetchone()
        if not mismatch:
            con.rollback()
            return {"ok": False, "reason": "no_exact_pending_match"}
        con.execute("UPDATE deposits SET status='rejected' WHERE id=? AND status='pending'", (mismatch["id"],))
        processing_id = int(mismatch["processing_message_id"] or 0)
        con.commit()
        row = mismatch
        message = _payment_result_message("rejected", row, provider=provider,
            reason="Payment SMS পাওয়া গেছে, কিন্তু Transaction ID আপনার জমা দেওয়া Transaction ID-এর সাথে মেলেনি।")
        if not _telegram_edit_message(row["user_id"], processing_id, message):
            _telegram_send_message(row["user_id"], message)
        return {"ok": True, "status": "rejected", "deposit_id": row["id"], "user_id": row["user_id"], "amount": row["amount"], "provider": provider, "trx_id": trx_id, "reason": "trx_mismatch"}
    except Exception:
        con.rollback()
        logger.exception("Auto payment processing failed")
...
class PaymentWebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not PAYMENT_WEBHOOK_SECRET:
            self._json_response(503, {"ok": False, "error": "webhook_secret_not_configured"})
            return

        provided_secret = self.headers.get("X-Webhook-Secret", "")
        if provided_secret != PAYMENT_WEBHOOK_SECRET:
        ...
        try:
            ...
            sms = str(data.get("message") or data.get("sms") or "").strip()
            provider = str(data.get("provider") or data.get("sender") or "").strip()
...
async def deposit_trx(update, context, trx):
    ...
    for admin_id in ADMIN_IDS:
        try:
            await context.bot.send_message(
                admin_id,
                premiumize_html(f"💳 <b>NEW DEPOSIT</b>\n\n"
    ...
    processing = await update.message.reply_text(
        premiumize_html(
            ...
            f"💰 Amount: <b>{money(amount)} ৳</b>\n"
            f"🧾 Transaction ID: <code>{escape(trx)}</code>\n\n"
            "আপনার payment যাচাই করা হচ্ছে। অনুগ্রহ করে অপেক্ষা করুন।\n"
            "Transaction ID match হলে payment automatically approve হবে।"
        ), parse_mode="HTML", reply_markup=main_keyboard(user.id)
    )
    con = db()
    con.execute("UPDATE deposits SET processing_message_id=? WHERE id=?", (processing.message_id, deposit_id))
    con.commit(); con.close()
    context.user_data.clear()
...
def _parse_xlsx_rows(raw_bytes):
    ...
    No openpyxl installation is required.
    The first worksheet containing data is used.
    """
    ns = {"x": "http://schemas.openxmlformats.org/spreadsheetml/2006/main"}
...
def main():
    ...
    if AUTO_PAYMENT_ENABLED:
        if not PAYMENT_WEBHOOK_SECRET:
            raise RuntimeError(
                "AUTO_PAYMENT_ENABLED=true হলে PAYMENT_WEBHOOK_SECRET সেট করতে হবে।"
            )
