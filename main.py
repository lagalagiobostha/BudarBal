    ...
    if data.startswith("deposit_approve:") or data.startswith("deposit_reject:"):
        ...
        if action == "deposit_approve":
            ...
            try:
                await context.bot.send_message(
                    row["user_id"],
                    premiumize_html(deposit_approved_text(row, get_balance(row["user_id"]))),
                    parse_mode="HTML"
                )
            except Exception:
                pass
            ...
            try:
                await context.bot.send_message(
                    row["user_id"],
                    premiumize_html(deposit_rejected_text(row)),
                    parse_mode="HTML"
                )
            except Exception:
                pass
...
def _telegram_delete_message(chat_id, message_id):
    """Delete a previously sent Telegram message from the payment webhook thread."""
    ...
    try:
        ...
        logger.exception("Telegram processing-message deletion failed")
        return False


def deposit_approved_text(row, new_balance):
    return (
        f"🎉 <b>Deposit Approved!</b>\n\n"
        f"💰 <b>Amount:</b> ৳{money(row['amount'])}\n"
        f"💳 <b>Method:</b> {escape(row['payment_method'] or '—')}\n"
...
def deposit_rejected_text(row):
    return (
        f"❌ <b>Deposit Rejected</b>\n\n"
        f"💰 <b>Amount:</b> ৳{money(row['amount'])}\n"
        f"💳 <b>Method:</b> {escape(row['payment_method'] or '—')}\n\n"
        f"ℹ️ পেমেন্ট তথ্য সঠিকভাবে যাচাই করা যায়নি। প্রয়োজনে আবার Deposit Submit করুন।"
    )
...
def auto_approve_payment(provider, amount, trx_id, raw_sms):
    ...
    # Remove the user's processing message before sending the final result.
    _telegram_delete_message(row["user_id"], row["pending_message_id"])

    # User notification — identical to manual approval.
    _telegram_send_message(
        row["user_id"],
        premiumize_html(deposit_approved_text(row, new_balance))
    )
...
async def deposit_trx(update, context, trx):
    ...
    # Show a single processing message while the deposit is being verified.
    context.user_data.clear()
    ...
    con = db()
    con.execute(
        "UPDATE deposits SET pending_message_id=? WHERE id=?",
        (processing_msg.message_id, deposit_id)
    )
    con.commit()
    ...
    # If auto-payment completed during submission, remove Processing immediately
    # and show the same approved message used by manual approval.
    if current_status == "approved":
        try:
            await context.bot.delete_message(
                chat_id=user.id, message_id=processing_msg.message_id
            )
        except Exception:
            pass
        row_for_message = {
        ...
        await update.message.reply_text(
            premiumize_html(deposit_approved_text(row_for_message, get_balance(user.id))),
...
async def approve_deposit(update, context):
    ...
    try:
        if row["pending_message_id"]:
            await context.bot.delete_message(
                chat_id=row["user_id"], message_id=row["pending_message_id"]
            )
    except Exception:
        pass
    try:
...
async def reject_deposit(update, context):
    ...
    if cur.rowcount:
        try:
            pending_row = db().execute("SELECT * FROM deposits WHERE id=?", (did,)).fetchone()
            # The row remains available for the notification; status is already rejected.
            if pending_row and pending_row["pending_message_id"]:
                await context.bot.delete_message(
                    chat_id=pending_row["user_id"], message_id=pending_row["pending_message_id"]
                )
            if pending_row:
                await context.bot.send_message(
                    pending_row["user_id"],
                    premiumize_html(deposit_rejected_text(pending_row)),
                    parse_mode="HTML"
                )
            db().close()
        except Exception:
