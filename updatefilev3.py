...
PAYMENT_WEBHOOK_SECRET = os.getenv("PAYMENT_WEBHOOK_SECRET", "free-auto-pay-2026-bd-v2")
...
def parse_payment_sms(message, provider=""):
    ...
    if p not in ("bkash", "nagad"):
        if "bkash" in low or "b-kash" in low:
            ...
            # Some relays provide only an SMS sender ID. Keep this mapping
            # conservative so an unrelated SMS cannot approve a deposit.
            sender = str(provider or "").strip().lower()
            if sender in {"bkash", "bkash-payment", "bkashpay"}:
                p = "bkash"
            elif sender in {"nagad", "nagad-payment", "nagadpay"}:
                p = "nagad"
...
class PaymentWebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            logger.info(
                "Payment SMS parsed: provider=%s amount=%s trx_id=%s",
                parsed_provider, amount, trx_id
            )
